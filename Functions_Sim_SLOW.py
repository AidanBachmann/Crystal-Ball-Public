...
def makePosPlotsMercator(particles,Nt,N,time,r): # Make plots of particle position in (φ,θ) plane
    ...
    for i in np.linspace(0,Nt,Nt+1).astype('int'):
        if not i%10:
            ...
            #plt.savefig('z_Data_5/Movie/step' + "{:05d}".format(i+1),bbox_inches='tight') # Save figure
...
def plotEnergyAll(particles,Nt,time,N,Nrm,NMC,r,m,data_dir,log=False,save=False):
    ...
    #ax[:,:].xaxis.labelpad = 10 # Add padding between labels and plot
    ...
    #fig = plt.figure(figsize=(12,10))
    #pressure = U/((4/3)*np.pi*pow(r,3))*(1.602176634e-19) # Estimate pressure in J/m^3
    #plt.plot(time,pressure)
    #plt.xlabel('Time (s)')
    #plt.ylabel(r'Pressure ($\frac{J}{m^3}$)')
    #plt.title('Estimate of System Pressure as a Function of Time')
    #plt.grid()
    ...
    #if save:
    #    if log:
    #        plt.savefig(f'Output Sim/Lattice_{NMC}_N_{N}_Nrm_{Nrm}_Plog.png',bbox_inches='tight',format='png')
    #    else:
    #        plt.savefig(f'Output Sim/Lattice_{NMC}_N_{N}_Nrm_{Nrm}_P.png',bbox_inches='tight',format='png')
    #plt.close(fig)
...
def simSingle(N,Nt,Nrm,k,r,m,dt,C,max_dt,pIdx,lattice,NMC,data_dir,save=True,log=False): # Single simulation
    ...
    #makeEnergySpatialPlots(simParticles,Nt,N,timeSeries,r,m,Tmax)
    
def simMC_Multi(N,Nt,Nrm,k,r,m,dt,C,max_dt,pIdx,lattice,NMC,data_dir): # NMC simulations on NMC processes
    ...
    for i in numProcs:
        args_ = (N,Nt,Nrm,k,r,m,dt,C,max_dt,pIdx,lattice,i,data_dir,)
