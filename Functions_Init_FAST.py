...
def analyze_drag_coeff(N,nu,r=25.5e-10,NN=6):
    ...
    for i in np.linspace(0,len(nu)-1,len(nu),dtype='int'):
        ...
        nu[i] = int(pow(10,i+1))
    plt.figure(figsize=(12,10))
    plt.scatter(nu,data[:,0],c='r')
    plt.errorbar(nu,data[:,0],xerr=None,yerr=data[:,1],ls='none',capsize=5,c='black')
    plt.xlabel('Damping Parameter')
    plt.ylabel('Average Lattice Constant (Å)')
    plt.title('Average Lattice Constant as a Function of Damping Parameter')
    plt.grid()
    plt.xscale('log')
    #plt.show()
    plt.savefig('d_nu.png',dpi=600)