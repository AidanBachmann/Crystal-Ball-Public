...
def finalTimeStepMercator(particles,N,time,r,data_dir,NMC=1):
    colors = plt.cm.hsv(np.linspace(0,1,N)) # Create unique color for each particle
    fig = plt.figure(figsize=(12,10)) # Set plot size
...
def firstTimeStepMercator(particles,N,r,data_dir,NMC=1):
    colors = plt.cm.hsv(np.linspace(0,1,N)) # Create unique color for each particle
...
def plotVtAll(particles,time,N,data_dir,log=False,save=False,NMC=1):
    fig,ax = plt.subplots(2,2,figsize=(12,10))
...
@jit(nopython = True)
def computeAllForce(particles,n,N,k,r,m,nu,t,tau): # Compute forces from all particles for sim, no drag term
    for i in np.linspace(0,N-1,N).astype('int'):
        ...
        drag = dragTerm(np.array([particles[i,n,3],particles[i,n,4],particles[i,n,5]]),nu,t,tau) # Compute drag force at current step
...
@jit(nopython = True)
def updateState(particles,n,N,k,r,m,nu,dt,t,tau): # Compute single step
    computeAllForce(particles,n,N,k,r,m,nu,t,tau)
...
def initLattice(particles,N,Nt,k,r,m,nu,dt,max_dt,C,tau): # Initialize lattice
    ...
    for i in np.linspace(0,Nt-1,Nt).astype('int'):
        t += dt
        updateState(particles,i,N,k,r,m,nu,dt,t,tau)
        time[i+1] = time[i] + dt
...
def initSingle(N,Nt,NN,n,particleIdx,k,r,m,nu,dt,max_dt,C,NMC,seed,data_dir,write=True,save=True,log=False,tau=1): # Init single lattice with N particles
    ...
    timeSeries = initLattice(particles,N,Nt,k,r,m,nu,dt,max_dt,C,tau) # Run simulation
...
def initMC_Multi(N,Nt,NN,n,particleIdx,k,r,m,nu,dt,max_dt,C,NMC,seed,data_dir,write,save,log,tau): # Init NMC lattices for constant N in parallel
    ...
    for i in numProcs:
        args_ = (N,Nt,NN,n,particleIdx,k,r,m,nu,dt,max_dt,C,i,seed+i,data_dir,write,save,log,tau,)
